"""CLI entrypoint for AR checkpoint evaluation."""
...
from train import init_ar_train_state
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Evaluate a saved AR checkpoint on a dataset split")
...
def _restore_state(args: argparse.Namespace, target: np.ndarray):
    ...
    method = Autoregressive(
        ...
        trajectory_points_num=int(target.shape[1]),
        use_time=False,
    )
    state_template = init_ar_train_state(
        method,
        seed=0,
    )
...
def main() -> None:
    ...
    pred = method.inference(state, target[:, 0])
